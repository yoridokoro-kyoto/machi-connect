...
type CsvRow = {
  ...
  isDuplicate: boolean
  duplicateId: string | null  // 重複している既存世帯のID
...
export default function HouseholdsPage() {
  ...
  const [importResult, setImportResult] = useState<{ add: number; overwrite: number; skip: number } | null>(null)
  ...
  const handleInvite = async (h: Household) => {
    ...
    if (error) { alert('招待メールの送信に失敗しました') }
    else { setInviteSuccess(h.id); setTimeout(() => setInviteSuccess(null), 3000) }
    setInviting(null)
  }
    ...
    reader.onload = (event) => {
      try {
        ...
        rows.forEach((line, i) => {
          ...
          if (!name?.trim()) { errors.push(`${i + 2}行目: 世帯主氏名が空です`); return }
          if (!address?.trim()) { errors.push(`${i + 2}行目: 住所が空です`); return }

          const data: HouseholdFormData = {
          ...
          parsed.push({
            ...
            action: existing ? 'skip' : 'add', // 重複はデフォルトでスキップ
          })
        })
  ...
  // 行のアクションを変更
  const updateRowAction = (index: number, action: CsvRow['action']) => {
    setCsvRows(prev => prev.map((row, i) => i === index ? { ...row, action } : row))
  }
  ...
  const handleCsvImport = async () => {
    ...
    for (const row of csvRows) {
      if (row.action === 'skip') { skipCount++; continue }
      if (row.action === 'overwrite' && row.duplicateId) {
        const { error } = await updateHousehold(row.duplicateId, row.data)
        if (!error) overwriteCount++
      } else if (row.action === 'add') {
        const { error } = await createHousehold(row.data)
        if (!error) addCount++
      }
    }

    setImportResult({ add: addCount, overwrite: overwriteCount, skip: skipCount })
    setImporting(false)
    await load()
    setTimeout(() => { setMode('list'); setImportResult(null); setCsvRows([]) }, 2500)
  }
  ...
  const duplicateCount = csvRows.filter(r => r.isDuplicate).length
    ...
    return (
      <div style={{ padding: '20px 16px', maxWidth: '760px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
          <button onClick={() => { setMode('list'); setCsvRows([]) }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '16px' }}>
          {[
            { label: '新規登録', count: newCount, bg: '#EAF3DE', color: '#3B6D11' },
            { label: '重複あり', count: duplicateCount, bg: '#FFF8E6', color: '#8B6000' },
            { label: '合計', count: csvRows.length, bg: '#E6F1FB', color: '#0C447C' },
          ].map(item => (
            <div key={item.label} style={{ background: item.bg, borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '12px', color: item.color, marginTop: '2px' }}>{item.label}</div>
            </div>
          ))}
        </div>

        {duplicateCount > 0 && (
          <div style={{ background: '#FFF8E6', color: '#8B6000', padding: '12px 14px', borderRadius: '8px', marginBottom: '16px', fontSize: '13px' }}>
            ⚠️ {duplicateCount}件が既存の住所と重複しています。行ごとに「スキップ」または「上書き」を選択してください。
          </div>
        )}

        {importResult && (
          <div style={{ background: '#EAF3DE', color: '#3B6D11', padding: '14px', borderRadius: '8px', marginBottom: '16px', fontSize: '14px' }}>
            ✅ 完了：新規登録 {importResult.add}件 / 上書き {importResult.overwrite}件 / スキップ {importResult.skip}件
          </div>
        )}
              ...
              <tbody>
                {csvRows.map((row, i) => (
                  <tr key={i} style={{
                    ...
                    background: row.action === 'skip' ? '#f5f5f5' : row.isDuplicate ? '#FFFBF0' : i % 2 === 0 ? '#f9fbff' : '#fff',
                    opacity: row.action === 'skip' ? 0.6 : 1,
                  }}>
                    <td style={{ padding: '10px 12px', whiteSpace: 'nowrap' }}>
                      {row.isDuplicate
                        ? <span style={{ background: '#FFF8E6', color: '#8B6000', fontSize: '11px', padding: '2px 6px', borderRadius: '4px' }}>⚠️ 重複</span>
                        : <span style={{ background: '#EAF3DE', color: '#3B6D11', fontSize: '11px', padding: '2px 6px', borderRadius: '4px' }}>✅ 新規</span>
                      }
                    </td>
                    <td style={{ padding: '10px 12px', color: '#888' }}>{row.data.household_no || '―'}</td>
                    <td style={{ padding: '10px 12px', fontWeight: '500' }}>{row.data.name}</td>
                    <td style={{ padding: '10px 12px' }}>{row.data.address}</td>
                    <td style={{ padding: '10px 12px', color: '#185FA5' }}>{row.data.email || '―'}</td>
                    <td style={{ padding: '10px 12px' }}>{row.data.phone || '―'}</td>
                    <td style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>
                      {row.isDuplicate ? (
                        <div style={{ display: 'flex', gap: '4px' }}>
                          <button onClick={() => updateRowAction(i, 'skip')}
                            style={{ padding: '4px 10px', fontSize: '12px', background: row.action === 'skip' ? '#555' : '#f5f5f5', color: row.action === 'skip' ? '#fff' : '#555', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer' }}>
                            スキップ
                          </button>
                          <button onClick={() => updateRowAction(i, 'overwrite')}
                            style={{ padding: '4px 10px', fontSize: '12px', background: row.action === 'overwrite' ? '#185FA5' : '#f5f5f5', color: row.action === 'overwrite' ? '#fff' : '#555', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer' }}>
                            上書き
                          </button>
                        </div>
                      ) : (
                        <span style={{ fontSize: '12px', color: '#3B6D11' }}>登録する</span>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button onClick={handleCsvImport} disabled={importing}
            ...
            {importing ? '登録中...' : `✅ インポートを実行する（${csvRows.filter(r => r.action !== 'skip').length}件）`}
          </button>
        </div>
      </div>
    )
  ...
  if (mode === 'add' || mode === 'edit') {
    return (
      <div style={{ padding: '20px 16px', maxWidth: '600px', margin: '0 auto' }}>
        ...
        {error && <div style={{ background: '#FCEBEB', color: '#A32D2D', padding: '14px', borderRadius: '8px', marginBottom: '16px', fontSize: '14px' }}>{error}</div>}
        <div style={{ background: '#fff', borderRadius: '12px', padding: '20px', border: '0.5px solid #e0e0e0', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            ...
            <input value={form.household_no} onChange={(e) => setForm({ ...form, household_no: e.target.value })} placeholder="例：1組-1番" style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>世帯主氏名 <span style={{ color: '#A32D2D' }}>*</span></label>
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="例：山田 太郎" style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>住所 <span style={{ color: '#A32D2D' }}>*</span></label>
            <input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder="例：京都市○○区○○町1-1" style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>メールアドレス <span style={{ fontSize: '12px', color: '#888', fontWeight: '400' }}>（任意・招待メール送信に使用）</span></label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="例：yamada@example.com" style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>電話番号 <span style={{ fontSize: '12px', color: '#888', fontWeight: '400' }}>（任意）</span></label>
            <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="例：090-1234-5678" style={inputStyle} />
          </div>
    ...
    <div style={{ minHeight: '100%' }}>
      ...
      <div style={{ padding: '12px 16px', maxWidth: '600px', margin: '0 auto' }}>
        <div style={{ background: '#f8fafc', border: '1px solid #e0e8f0', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div style={{ fontSize: '14px', fontWeight: '500', color: '#555' }}>📥 CSVで一括登録・更新</div>
          ...
          <div style={{ fontSize: '12px', color: '#aaa' }}>
            CSV列順：世帯番号, 世帯主氏名*, 住所*, メールアドレス, 電話番号　（*必須）／ 住所が一致する場合は重複とみなします
          </div>
        </div>
      </div>
