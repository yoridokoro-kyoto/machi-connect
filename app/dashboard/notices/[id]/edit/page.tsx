...
import { fetchNoticeById, updateNotice } from '@/features/notices'
...
export default function EditNoticePage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    setError('')
    await updateNotice(params.id as string, { title, content, type, requiresConfirmation, requiresRsvp, rsvpDeadline: rsvpDeadline || null })
    router.push(`/dashboard/notices/${params.id}`)
  }

  const toggleStyle = (active: boolean) => ({
    flex: 1, padding: '10px', fontSize: '14px', fontWeight: '500' as const,
    background: active ? '#185FA5' : '#f5f5f5', color: active ? '#fff' : '#555',
    border: '1.5px solid' as const, borderColor: active ? '#185FA5' : '#ddd',
  ...
  return (
    <div style={{ padding: '20px 16px', maxWidth: '600px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
        <button onClick={() => router.push(`/dashboard/notices/${params.id}`)} style={{ background: '#f5f5f5', border: '1px solid #ddd', color: '#555', fontSize: '14px', padding: '8px 14px', borderRadius: '8px', cursor: 'pointer' }}>
          ← 詳細に戻る
        </button>
        <span style={{ fontSize: '18px', fontWeight: '500' }}>✏️ 投稿を編集</span>
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: '12px', padding: '20px', border: '0.5px solid #e0e0e0' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {(['notice', 'circular'] as const).map((t) => (
              <button key={t} onClick={() => setType(t)} style={{ flex: 1, padding: '12px', fontSize: '15px', borderRadius: '8px', border: '1.5px solid', borderColor: type === t ? '#185FA5' : '#ddd', background: type === t ? '#E6F1FB' : '#fff', color: type === t ? '#185FA5' : '#888', cursor: 'pointer', fontWeight: type === t ? '600' : '400' }}>
                {t === 'notice' ? '📢 お知らせ' : '📋 回覧板'}
              </button>
            ))}
          </div>
        ...
        {type === 'circular' && (
          <div style={{ background: '#f8fafc', border: '1px solid #e0e8f0', borderRadius: '8px', padding: '16px', marginBottom: '20px' }}>
            ...
            <div>
              ...
              {requiresRsvp && (
                <input type="date" value={rsvpDeadline} onChange={(e) => setRsvpDeadline(e.target.value)} min={new Date().toISOString().split('T')[0]}
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="本文を入力" rows={10}
        ...
        <button onClick={handleSave} disabled={saving || !title.trim()} style={{ width: '100%', padding: '16px', fontSize: '17px', fontWeight: '600', background: saving || !title.trim() ? '#aaa' : '#185FA5', color: '#fff', border: 'none', borderRadius: '8px', cursor: saving || !title.trim() ? 'not-allowed' : 'pointer' }}>
          {saving ? '保存中...' : '💾 保存する'}
        </button>
      </div>
    </div>
