...
import { fetchNoticeById, fetchRsvpSummary } from '@/features/notices'
import type { Notice, RsvpRecord } from '@/features/notices'
...
export default function RsvpSummaryPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [noticeData, rsvpData] = await Promise.all([
        fetchNoticeById(params.id as string),
        fetchRsvpSummary(params.id as string),
      ])
  ...
  return (
    <div style={{ minHeight: '100%' }}>
      ...
      <div style={{ background: '#fff', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px', borderBottom: '0.5px solid #e0e0e0' }}>
        ...
        <span style={{ fontSize: '16px', fontWeight: '500' }}>📊 出欠集計</span>
      </div>

      <div style={{ padding: '20px 16px', maxWidth: '600px', margin: '0 auto' }}>
        {notice && (
          <div style={{ background: '#fff', borderRadius: '12px', padding: '16px', marginBottom: '16px', border: '0.5px solid #e0e0e0' }}>
            <div style={{ fontSize: '16px', fontWeight: '500', marginBottom: '4px' }}>{notice.title}</div>
            {notice.rsvp_deadline && <div style={{ fontSize: '13px', color: '#888' }}>回答期限：{formatDate(notice.rsvp_deadline)}</div>}
          </div>
        )}
          ...
          <div key={label} style={{ background: '#fff', borderRadius: '12px', padding: '16px', marginBottom: '12px', border: '0.5px solid #e0e0e0' }}>
            ...
            {list.map((r) => (
              <div key={r.user_id} style={{ display: 'flex', alignItems: 'center', padding: '8px 0', borderBottom: '0.5px solid #f0f0f0' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', marginRight: '10px' }}>👤</div>
                <div style={{ fontSize: '15px' }}>{r.name}</div>
              </div>
            ))}
          </div>
        ))}

        {records.length === 0 && <div style={{ textAlign: 'center', padding: '48px', color: '#888' }}>まだ回答がありません</div>}
      </div>
    </div>
  )
}
